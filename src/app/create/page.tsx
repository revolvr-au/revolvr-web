      ...
      {previews.length === 0 ? (
        <>
          ...
          <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 10 }}>
            <div style={{ position: "absolute", top: 140, left: 20, color: "#00e5ff", fontSize: 10, fontFamily: "monospace", animation: "blink 1s infinite" }}>SYSTEM: ANALYZING TENSORS...</div>
            <div style={{ position: "absolute", left: 0, right: 0, height: 2, background: "rgba(0, 229, 255, 0.5)", boxShadow: "0 0 10px rgba(0, 229, 255, 0.8)", animation: "scanner 3s linear infinite" }} />
        ...
        <>
          ...
          <div style={{ position: "absolute", inset: 0, zIndex: 0, background: "#000" }}>
            {files[activeIndex]?.type.startsWith("video/") ? (
              <video src={previews[activeIndex]} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            ) : (
              <img src={previews[activeIndex]} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            )}
          </div>
          ...
          {/* BOTTOM CONTROLS (Caption & Submit) */}
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 10, display: "flex", flexDirection: "column", padding: "20px 20px calc(20px + env(safe-area-inset-bottom))" }}>
            ...
            {loading && uploadProgress > 0 && (
              <div style={{ width: "100%", height: 3, background: "rgba(255,255,255,0.1)", marginBottom: 16, borderRadius: 2, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${uploadProgress}%`, background: "#00e5ff", transition: "width 0.3s ease" }} />
              </div>
            )}
