...
export default function PublicFeedClient() {
  ...
  // PeopleRail: Featured list derived from latest posts (unique authors).
  // For now: use the author’s latest post image as the “avatar” tile.
  const peopleRail: PersonRailItem[] = useMemo(() => {
    const m = new Map<string, PersonRailItem>();
    ...
    for (const p of posts) {
      ...
      if (!m.has(email)) {
        m.set(email, {
          email,
          imageUrl: p.imageUrl,
          isVerified: verifiedSet.has(email),
        });
      }
    }
  ...
  return (
    <FeedLayout>
      {/* People rail (Step 1: Featured List) */}
      <PeopleRail title="Featured creators" people={peopleRail} />
